...
\libfunction
  {readLn}
  {Void -> IO \; String}
  {Reads a complete line from the standard input. A line ends with either a linefeed (\textbackslash n) or a carriage return (\textbackslash r).}

\libfunction
  {writeLn}
  {String -> IO \; Void}
  {Prints the string to the standard output, followed by a linefeed (\textbackslash n).}
