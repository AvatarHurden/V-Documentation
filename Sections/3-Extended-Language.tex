...
For composing multiple IO operations, using the basic bind and return operations is cumbersome.
Because of this, the following notation was introduced:
...
\subsubsection{Do Notation Translation}

\infax[Tr-Do-Base]
  {\text{concat}(e', \gamma) = e', \gamma}

\infrule[Tr-Do-Bind]
  {\text{concat}(doTerms, \gamma) = f', \gamma'}
  {\text{concat}(p' \leftarrow e' :: doTerms, \gamma) = \text{bind} \; e' \; (\texttt{fn} \; p' \Rightarrow f'), \gamma'}

\infrule[Tr-Do-Term]
  {\text{concat}(doTerms, \gamma) = f', \gamma'}
  {\text{concat}(e' :: doTerms, \gamma) = \text{bind} \; e' \; (\texttt{fn} \; \_ \Rightarrow f'), \gamma'}

\infrule[Tr-Do-Decl]
  {\text{concat}(doTerms, \gamma) = f', \gamma'}
  {\text{concat}(decl' :: doTerms, \gamma) = \texttt{let} \; decl' \; \texttt{in} \; f', \gamma'}

\infrule[Tr-E-Do]
  {\text{concat}(doTerms) = f', \gamma' \andalso \gamma' \vdash f' \Rightarrow f}
  {\gamma \vdash \texttt{do} \; doTerms \Rightarrow f}
